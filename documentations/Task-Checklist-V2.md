...
# Pendahuluan
Dokumen ini adalah level rincian paling granular dari seluruh rangkaian dokumentasi proyek, mengikuti struktur Epic → Feature → Task → Subtask → Checklist. Epic mengikuti rilis pada Development Roadmap (Fase 8); Feature mengikuti sprint pada Sprint Breakdown (Fase 9); setiap Task dilengkapi Deskripsi, Acceptance Criteria, Dependency, Priority, Estimasi Kesulitan, Estimasi Waktu, dan Definition of Done, diikuti Subtask dan Checklist Verifikasi yang dapat langsung dicentang selama implementasi.
Total 35 Task tercakup pada dokumen ini, mencakup seluruh fitur inti hingga fitur lanjutan yang telah disepakati sepanjang Fase 0-9, termasuk keputusan terbaru: dukungan poll multiple-choice (T16.1), operasi bulk dengan permission check per-item (T18.1), **EPIC 8 — Direct Message** (T20.1-T20.3), serta perluasannya **Friend & Social System** (T21.1-T21.4) dan **DM Calls & Group Management** (T22.1-T22.2) — lihat Addendum-Direct-Message.docx (v2) untuk detail spesifikasi lengkap.
...
**T20.1 — Direct Message Data Model & Conversation Management**
| **Deskripsi** | **Menambahkan model data untuk percakapan Direct Message (1:1 dan group DM) dengan menjadikan DM sebagai channel khusus tanpa server_id, ditambah tabel dm_participants untuk anggotanya. Pendekatan ini dipilih agar seluruh infrastruktur Messaging (pesan, reaksi, pin, attachment, WebSocket) dapat dipakai ulang tanpa modul terpisah.** |
| --- | --- |
| **Acceptance Criteria** | **1. channels.server_id dibuat nullable dan channels.type mendukung nilai baru 'dm' dan 'group_dm'.  2. Memulai DM 1:1 baru dengan user yang sama mengembalikan conversation yang sudah ada (tidak duplikat).  3. Group DM mendukung lebih dari 2 participant.** |
| **Dependency** | **T4.1, T2.1** |
...
| --- | --- |
| **Acceptance Criteria** | **1. Endpoint messaging yang sudah ada (kirim/edit/hapus/reaksi/pin/upload) berfungsi pada channel bertipe dm/group_dm tanpa duplikasi kode.  2. Middleware permission mengenali channel DM dan mengevaluasi akses berdasarkan dm_participants, bukan permissions_bitmask.  3. Broadcast WebSocket & Redis Pub/Sub bekerja sama seperti pada channel biasa.** |
| **Dependency** | **T20.1, T6.1, T7.2** |
| **Priority** | **Must** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **8 jam** |
| **Definition of Done** | **Kirim/terima pesan pada DM berfungsi realtime end-to-end, termasuk lintas instance.** |
**Subtask:**
- [ ] Update middleware evaluasi permission agar bercabang: channel biasa → bitmask, channel dm/group_dm → cek dm_participants.
- [ ] Verifikasi endpoint messages/reactions/pin/attachments bekerja tanpa modifikasi pada channel DM.
- [ ] Uji broadcast WebSocket & Redis Pub/Sub pada channel DM dengan 2 replika aplikasi.
**Checklist Verifikasi:**
- [ ] User yang bukan participant DM mendapat 403 saat mencoba mengakses channel tersebut
- [ ] Pesan DM tersiar realtime ke seluruh participant, termasuk lintas instance

**T20.3 — Integrasi Notifikasi & Pencarian untuk DM**
| **Deskripsi** | **Memastikan pesan DM baru memicu notifikasi (realtime/email) dan dapat ditemukan melalui fitur pencarian, hanya untuk participant terkait.** |
| --- | --- |
| **Acceptance Criteria** | **1. Pesan DM baru pada percakapan yang penerimanya offline memicu notifikasi email.  2. Hasil pencarian pesan DM hanya muncul bagi participant percakapan tersebut.** |
| **Dependency** | **T20.2, T9.1, T11.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Easy** |
| **Estimasi Waktu** | **5 jam** |
| **Definition of Done** | **Notifikasi & pencarian mencakup DM secara konsisten dengan aturan akses partisipan.** |
**Subtask:**
- [ ] Perluas trigger notifikasi (T9.1) agar berlaku juga untuk event pesan pada channel dm/group_dm.
- [ ] Pastikan query search (T11.1) memfilter pesan DM hanya untuk user yang menjadi dm_participants channel terkait.
**Checklist Verifikasi:**
- [ ] Notifikasi email terkirim untuk DM saat penerima offline
- [ ] Pesan DM tidak muncul di hasil pencarian user yang bukan participant

## Feature S21 — Friend & Social System
**T21.1 — Friend Request Lifecycle**
| **Deskripsi** | **Sistem kirim/terima/tolak/batalkan permintaan pertemanan, dengan auto-accept saat kedua pihak saling mengirim permintaan (mutual request).** |
| --- | --- |
| **Acceptance Criteria** | **1. Mengirim permintaan ke user yang sudah mengirim permintaan ke aktor otomatis menjadi status 'accepted'.  2. UNIQUE(user_one_id, user_two_id) mencegah row friendships duplikat meski request dikirim dari kedua arah nyaris bersamaan.** |
| **Dependency** | **T2.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **8 jam** |
| **Definition of Done** | **Alur pertemanan lengkap (request/accept/decline/cancel/remove) berfungsi sesuai SRS-DM-03.** |
**Subtask:**
- [ ] Prisma schema friendships (user_one_id < user_two_id canonical, status, action_user_id).
- [ ] Endpoint POST /friends/requests dengan logic auto-accept mutual request.
- [ ] Endpoint PATCH /friends/requests/{id}/accept, DELETE /friends/requests/{id}, DELETE /friends/{userId}.
- [ ] Endpoint GET /friends dengan filter ?tab=online/all/pending/blocked.
**Checklist Verifikasi:**
- [ ] Mutual request otomatis menjadi accepted tanpa race condition (diuji manual dengan request paralel)
- [ ] Tab pending menampilkan permintaan masuk & keluar secara terpisah

**T21.2 — Block User & DM Privacy Settings**
| **Deskripsi** | **Endpoint blokir/buka blokir pengguna dan pengaturan privasi siapa yang boleh mengirim DM/permintaan pertemanan.** |
| --- | --- |
| **Acceptance Criteria** | **1. Memblokir user otomatis menghapus relasi pertemanan yang ada.  2. User yang diblokir menerima 403 generik saat mencoba mengirim DM/friend request, tanpa disebutkan bahwa ia diblokir.** |
| **Dependency** | **T21.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Block & privacy setting ditegakkan konsisten pada seluruh alur DM/friend request sesuai SRS-DM-04.** |
**Subtask:**
- [ ] Prisma schema blocked_users & kolom users.dm_privacy.
- [ ] Endpoint POST/DELETE /users/{userId}/block.
- [ ] Endpoint PATCH /users/me/privacy.
- [ ] Update middleware pembuatan DM (T20.1) agar cek blocked_users & dm_privacy sebelum membuat channel.
**Checklist Verifikasi:**
- [ ] User yang saling blokir tidak dapat memulai DM baru satu sama lain
- [ ] Privacy friends_only membuat DM dari non-teman berstatus pending_request, bukan ditolak keras

**T21.3 — Message Requests (Spam Filtering)**
| **Deskripsi** | **Penyaringan DM dari pengguna non-teman ke daftar 'Message Requests' terpisah, mengikuti SRS-DM-05.** |
| --- | --- |
| **Acceptance Criteria** | **1. DM baru dari non-teman ke user ber-privacy friends_only membuat dm_participants.status penerima = 'pending_request'.  2. GET /dm (v1) hanya mengembalikan percakapan berstatus 'accepted'.** |
| **Dependency** | **T21.2, T20.1** |
| **Priority** | **Should** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Message Requests terpisah dari DM List utama dan dapat diterima/ditolak.** |
**Subtask:**
- [ ] Tambah kolom dm_participants.status (migration, DEFAULT 'accepted').
- [ ] Update GET /dm (T20.1) agar memfilter status='accepted' — **audit ulang implementasi T20.1 yang sudah berjalan.**
- [ ] Endpoint GET /dm/requests, POST /dm/{channelId}/accept, DELETE /dm/{channelId}/decline.
**Checklist Verifikasi:**
- [ ] Message Request tidak muncul di GET /dm sebelum diterima
- [ ] Accept memindahkan percakapan ke GET /dm secara konsisten

**T21.4 — User Profile Endpoint**
| **Deskripsi** | **Endpoint data popup profil pengguna: bio, server bersama, teman bersama, dan status relasi terhadap aktor.** |
| --- | --- |
| **Acceptance Criteria** | **1. Response mencantumkan status relasi (none/pending/friends/blocked) relatif terhadap aktor yang meminta.  2. Query server/teman bersama tidak membebani database secara signifikan (dipertimbangkan caching Redis TTL pendek).** |
| **Dependency** | **T21.1** |
| **Priority** | **Could** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **GET /users/{userId}/profile mengembalikan data lengkap sesuai FR-DM-11.** |
**Subtask:**
- [ ] Tambah kolom users.bio.
- [ ] Endpoint GET /users/{userId}/profile (bio, mutual servers, mutual friends, relationship status).
- [ ] Pertimbangkan cache Redis TTL pendek untuk query mutual servers/friends.
**Checklist Verifikasi:**
- [ ] Data mutual servers & mutual friends akurat
- [ ] Response waktu wajar meski dipanggil berulang (diuji manual)

## Feature S22 — DM Calls & Group Management
**T22.1 — Voice/Video Call dalam DM**
| **Deskripsi** | **Endpoint token LiveKit untuk panggilan dalam konteks DM/Group DM, memakai ulang Voice/Video Module (T12.1) dengan sumber permission dm_participants.** |
| --- | --- |
| **Acceptance Criteria** | **1. Token hanya diterbitkan bagi participant aktif (status='accepted') pada channel DM terkait.  2. Room LiveKit dibuat berbasis channel_id DM, terpisah dari room channel server manapun.** |
| **Dependency** | **T12.1, T20.2** |
| **Priority** | **Could** |
| **Estimasi Kesulitan** | **Medium** |
| **Estimasi Waktu** | **6 jam** |
| **Definition of Done** | **Panggilan suara/video dalam DM berfungsi end-to-end sesuai SRS-DM-06.** |
**Subtask:**
- [ ] Endpoint POST /dm/{channelId}/call/token dengan pengecekan dm_participants.
- [ ] Verifikasi reuse penuh LiveKit Server SDK integration dari T12.1 tanpa duplikasi logic.
**Checklist Verifikasi:**
- [ ] Token DM call ditolak bagi non-participant
- [ ] Room DM terpisah dari room voice channel server manapun

**T22.2 — Manajemen Group DM**
| **Deskripsi** | **Endpoint ubah nama/ikon Group DM serta tambah/hapus peserta, tanpa hierarki Owner/Moderator (seluruh participant setara).** |
| --- | --- |
| **Acceptance Criteria** | **1. Hanya berlaku untuk channels.type='group_dm', ditolak untuk DM 1:1.  2. Menghapus diri sendiri dari participants = leave group & tidak menghapus channel bila masih ada participant lain.** |
| **Dependency** | **T20.1** |
| **Priority** | **Could** |
| **Estimasi Kesulitan** | **Easy** |
| **Estimasi Waktu** | **5 jam** |
| **Definition of Done** | **Group DM dapat dikelola penuh oleh peserta manapun sesuai SRS-DM-07.** |
**Subtask:**
- [ ] Endpoint PATCH /dm/{channelId} (rename/icon), khusus group_dm.
- [ ] Endpoint POST/DELETE /dm/{channelId}/participants.
- [ ] Broadcast event perubahan grup via WebSocket ke seluruh participant.
**Checklist Verifikasi:**
- [ ] Aksi manajemen grup pada DM 1:1 ditolak dengan error jelas
- [ ] Leave group tidak menghapus channel selama masih ada participant lain
...
Direct Message (EPIC 8) dimodelkan sebagai channel khusus (type dm/group_dm, server_id nullable) dengan tabel dm_participants terpisah, agar dapat memakai ulang seluruh infrastruktur Messaging/WebSocket yang sudah ada tanpa modul baru.
Fitur sosial nice-to-have (Friend, Block, Privacy, Message Requests, Call dalam DM, Group DM Management) ditambahkan sebagai Feature S21-S22 dengan prioritas Should/Could — DM 1:1 dasar (S20) tetap prioritas Must yang harus selesai lebih dahulu.
...
Apakah Group DM (T20.1) benar-benar dibutuhkan pada rilis ini atau dapat ditunda — mengingat baru ditambahkan belakangan dan belum melalui proses prioritisasi MoSCoW seperti fitur lain di PRD.
# Risiko Desain
...
EPIC 8 (Direct Message) ditambahkan setelah sebagian besar Epic 1-6 selesai diimplementasikan; mengubah channels.server_id menjadi nullable (T20.1) berisiko memerlukan penyesuaian pada query/permission check yang sudah ditulis sebelumnya dan mengasumsikan server_id selalu ada.
...
Dokumen fase sebelumnya (PRD, SRS, Database Design, API Specification, Architecture Document, UI/UX Specification) belum diperbarui secara penuh untuk mencerminkan Direct Message — sengaja dirangkum dalam satu dokumen Addendum terpisah alih-alih meregenerasi keenam dokumen tersebut, agar progres implementasi yang sudah tercatat pada checklist ini tidak terganggu.
